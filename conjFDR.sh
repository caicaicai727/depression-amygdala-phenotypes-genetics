## ------------------------------------------------------------------------------------
## Generate configuration and MATLAB run files
## ------------------------------------------------------------------------------------

sigfile=/cFDR/MDD2025_list
...
new_traitname1=$IDP
new_traitfiles=$disease".mat"
new_traitnames=$disease
new_outputdir="/cFDR/MDD2025/results/"$IDP"_"$name
...
cp -f software/pleiofdr/config_default.txt \
"/cFDR/config_"$IDP"_"$name"_cjfdr.txt"

cp -f software/pleiofdr/runme.m \
"/cFDR/runme_"$IDP"_"$name"_cjfdr.m"
...
sed -i "4s|config=.*|config='$newconfig'|g" \
"/cFDR/runme_"$IDP"_"$name"_cjfdr.m"
...
## ------------------------------------------------------------------------------------
## Run conjunctional FDR analyses
## ------------------------------------------------------------------------------------
...
echo -e $cmd > "/cFDR/MDD2025/a"$IDP"_"$name"_cfdr.sh"

qsub -q cluster40 \
-e "/cFDR/MDD2025/a"$IDP"_"$name"_cfdr.err" \
-o "/cFDR/MDD2025/a"$IDP"_"$name"_cfdr.out" \
"/cFDR/MDD2025/a"$IDP"_"$name"_cfdr.sh"

done


## ------------------------------------------------------------------------------------
## Convert results and identify independent lead SNPs
## ------------------------------------------------------------------------------------
...
anaconda/anaconda3/envs/ldsc/bin/python2 \
software/pleiofdr/python_convert/fdrmat2csv.py \
--mat "/cFDR/MDD2025/results/"$IDP"_"$name"/result.mat" \
--ref software/pleiofdr/9545380.ref \
--out "/cFDR/MDD2025/results/"$IDP"_"$name"/result.mat.csv"

anaconda/anaconda3/envs/ldsc/bin/python2 \
software/pleiofdr/python_convert/sumstats.py clump \
--clump-field FDR \
--force \
--plink plink \
--sumstats "/cFDR/MDD2025/results/"$IDP"_"$name"/result.mat.csv" \
--bfile-chr software/pleiofdr/chr@ \
--exclude-ranges '6:25119106-33854733' '8:7200000-12500000' \
--clump-p1 0.05 \
--out "/cFDR/MDD2025/results/"$IDP"_"$name"/result_FUMA_leadsnp.csv"
